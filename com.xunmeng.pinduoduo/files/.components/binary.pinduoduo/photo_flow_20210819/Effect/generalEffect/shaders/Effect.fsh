...
vec4 imageFlow() {
         ...
         for(int i = 0; i < 5; ++i)
         {
            ...
            allHeight = allHeight + realImageUVHeight[i] - blendHeightArray[i];
         }
         allHeight = allHeight + realImageUVHeight[5] - 1.0;
         float checkHeight = step(0.0, allHeight);
         allHeight = allHeight * checkHeight;
         float posBias = iTime * allHeight * 1.0;
