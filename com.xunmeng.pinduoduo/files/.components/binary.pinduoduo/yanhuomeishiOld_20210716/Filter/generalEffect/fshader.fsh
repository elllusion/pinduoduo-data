...
vec4 clut(vec4 textureColor, sampler2D textureLut) {
    ...
    vec2 texPos1;
    float textureColorR = 0.0009765625 + (0.123046875 * textureColor.r); //0.5/512.0 + ((0.125 - 1.0/512.0) * textureColor.r)
    float textureColorG = 0.0009765625 + (0.123046875 * textureColor.g); //0.5/512.0 + ((0.125 - 1.0/512.0) * textureColor.g)
    texPos1.x = (quad1.x * 0.125) + textureColorR;
    texPos1.y = (quad1.y * 0.125) + textureColorG;
    
    vec2 texPos2;
    texPos2.x = (quad2.x * 0.125) + textureColorR;
    texPos2.y = (quad2.y * 0.125) + textureColorG;
